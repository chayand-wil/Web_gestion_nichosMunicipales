...
fetch('../../autos.json')
    ...
    .then(data => {
        // Agrupar los autos por carril de inicio para espaciarlos
        const autosPorCarril = {};
        
        // Agrupar autos por carril de inicio
        ...
        // Para cada carril, crear autos espaciados
        Object.keys(autosPorCarril).forEach(carril => {
            ...
            autosEnCarril.forEach((autoConfig, index) => {
                ...
                // Determinar si el auto es horizontal o vertical inicialmente
                const esHorizontal = autoConfig.carrilInicio <= 8;
                
                // Calcular el espaciado según la dirección
                let offsetX = 0;
                let offsetY = 0;
                const espaciado = 100; // Espaciado entre autos en el mismo carril
                
                switch (carrilInicio.direccion) {
                    case "derecha":
                        offsetX = -espaciado * index;
                        break;
                    case "izquierda":
                        offsetX = espaciado * index;
                        break;
                    case "abajo":
                        offsetY = -espaciado * index;
                        break;
                    case "arriba":
                        offsetY = espaciado * index;
                        break;
                ...
                // Crear el auto con posición ajustada
                autos.push({
